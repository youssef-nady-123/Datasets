/* ============================================================================
   ...
   FIX LOG (what was broken in the original draft and why)
     - usp_etl_load_dim_date used TRUNCATE TABLE dwh.dim_date. SQL Server
       refuses to TRUNCATE any table that is the target of an active FOREIGN
       KEY constraint — and dwh.fact_sales.date_key references
       dwh.dim_date.date_key. That FK exists the moment fact_sales is
       created (Section 3), so the very first call to
       dwh.usp_etl_run_all failed on its first line, before anything else
       in the ETL ever executed. Fixed by switching that one statement to
       DELETE FROM dwh.dim_date, which respects FK constraints the same way
       TRUNCATE would if it were allowed, just logged row-by-row instead of
       deallocating pages. No identity reseed is needed because date_key is
       not an IDENTITY column. See Section 4.1 below for the fix in place.
   ============================================================================ */
...
-- ---- fact_sales ---------------------------------------------------------
-- NOTE: this table's FKs are exactly why dim_date can never be TRUNCATEd
-- once fact_sales exists — see the fix note in Section 4.1 below.
...
BEGIN
    ...
    -- FIX: the original script used TRUNCATE TABLE dwh.dim_date here.
    -- SQL Server blocks TRUNCATE on any table that is the target of a
    -- FOREIGN KEY (dwh.fact_sales.date_key references this table), so that
    -- statement raised an error the moment this proc ran — and since this
    -- is the first proc dwh.usp_etl_run_all calls, the whole ETL chain
    -- never got past step one. DELETE is FK-safe and does the same job
    -- (dim_date has no IDENTITY column, so there's no seed to reset).
...
BEGIN
    ...
    -- fact_sales has no incoming FKs from any other table, so TRUNCATE is
    -- safe here (unlike dim_date above) — kept as TRUNCATE for speed.
